...
const FEATURES = [
  ...
  { color:'rgba(93,232,180,0.09)', title:'Análisis de creativos', desc:'Hook rate, hold rate, CTR y conversión por anuncio. Sabes exactamente qué creativo está jalando y cuál está quemado.', tag:'Creativos', icon:<svg viewBox="0 0 24 24" fill="none" stroke="#5de8b4" strokeWidth="1.8" width="17" height="17"><path d="M15 10l-4 4-2-2"/><circle cx="12" cy="12" r="9"/></svg> },
  { color:'rgba(239,159,39,0.09)', title:'Reportes para clientes', desc:'Genera PDFs con tu branding en un clic. Programa envíos automáticos semanales o mensuales directamente a tu cliente.', tag:'White-label', icon:<svg viewBox="0 0 24 24" fill="none" stroke="#ef9f27" strokeWidth="1.8" width="17" height="17"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg> },
  { color:'rgba(55,138,221,0.1)', title:'Segmentación demográfica', desc:'Rendimiento por edad, género y ubicación en todas las plataformas. Sin exportar a Excel, sin armarlo tú a mano.', tag:'Audiencias', icon:<svg viewBox="0 0 24 24" fill="none" stroke="#85b7eb" strokeWidth="1.8" width="17" height="17"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg> },
  { color:'rgba(212,83,126,0.09)', title:'Alertas inteligentes', desc:'Te avisamos cuando el CPA sube, el presupuesto se agota o el ROAS cae. Actúa antes de que tu cliente te llame.', tag:'Automatización', icon:<svg viewBox="0 0 24 24" fill="none" stroke="#ed93b1" strokeWidth="1.8" width="17" height="17"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg> },
  { color:'rgba(99,153,34,0.09)', title:'Multi-cuenta', desc:'Gestiona todos tus clientes desde un panel. Cambia entre cuentas al instante sin desconectarte ni volver a hacer login.', tag:'Agencias', icon:<svg viewBox="0 0 24 24" fill="none" stroke="#97c459" strokeWidth="1.8" width="17" height="17"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2"/><line x1="12" y1="12" x2="12" y2="16"/><line x1="10" y1="14" x2="14" y2="14"/></svg> },
]
...
export default function Landing() {
  ...
  return (
    <div style={{fontFamily:DM,fontSize:'15px',lineHeight:'1.6',color:TEXT,background:BG,minHeight:'100vh'}}>
      <style>{`
        ...
        .kl .sh { font-family: 'Syne', sans-serif; font-size: 36px; font-weight: 700; line-height: 1.15; letter-spacing: -0.025em; color: #f0f0f2; margin-bottom: 12px; }
        .kl .sh1 { font-family: 'Syne', sans-serif; font-size: 42px; font-weight: 800; line-height: 1.1; letter-spacing: -0.03em; color: #f0f0f2; margin-bottom: 18px; }
        .kl a { text-decoration: none; cursor: pointer; }
        .nh:hover { color: #f0f0f2 !important; }
        .bh:hover { opacity: 0.88; }
      `}</style>
      ...
      <div className="kl">
        ...
        <nav style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'18px 32px',borderBottom:`0.5px solid ${BOR}`,background:'rgba(10,10,12,0.94)',backdropFilter:'blur(10px)',position:'sticky',top:0,zIndex:100}}>
          <div style={{display:'flex',alignItems:'center',gap:9,fontFamily:SYNE,fontWeight:700,fontSize:'17px',color:TEXT,cursor:'pointer'}} onClick={()=>router.push('/')}>
          ...
          <div style={{display:'flex',gap:26,fontSize:'14px'}}>
            {['Funciones','Plataformas','Precios','FAQ'].map(l=>(
              <a key={l} className="nh" style={{color:MUTED,fontFamily:DM}}>{l}</a>
            ))}
          </div>
          <div style={{display:'flex',gap:10,alignItems:'center'}}>
            <button className="bh" onClick={()=>router.push('/login')} style={{background:'none',border:`0.5px solid ${BOR}`,color:MUTED,padding:'8px 18px',borderRadius:8,fontSize:'13px',fontFamily:DM,cursor:'pointer'}}>Iniciar sesión</button>
            <button className="bh" onClick={()=>router.push('/registro')} style={{background:ACC,color:'white',border:'none',padding:'9px 20px',borderRadius:8,fontSize:'13px',fontWeight:500,fontFamily:DM,cursor:'pointer'}}>Empezar gratis</button>
          </div>
        </nav>

        {/* HERO */}
        <div style={{padding:'72px 32px 44px',textAlign:'center',maxWidth:760,margin:'0 auto'}}>
          <div style={{display:'inline-flex',alignItems:'center',gap:7,background:'rgba(124,110,245,0.09)',border:`0.5px solid rgba(124,110,245,0.25)`,color:ACC,fontSize:'12px',padding:'5px 13px',borderRadius:20,marginBottom:24,letterSpacing:'.02em',fontFamily:DM}}>
          ...
          <p style={{fontSize:'16px',color:MUTED,maxWidth:500,margin:'0 auto 30px',lineHeight:1.75,fontFamily:DM}}>
          ...
          <div style={{display:'flex',gap:11,justifyContent:'center',flexWrap:'wrap'}}>
            <button className="bh" onClick={()=>router.push('/registro')} style={{background:ACC,color:'white',border:'none',padding:'12px 24px',fontSize:'14px',fontWeight:500,borderRadius:9,display:'flex',alignItems:'center',gap:7,fontFamily:DM,cursor:'pointer'}}>
              <svg viewBox="0 0 16 16" fill="none" width="13" height="13"><path d="M8 2L13 8L8 14L3 8Z" fill="white"/></svg>
              Empezar gratis
            </button>
            <button style={{background:'none',border:`0.5px solid rgba(255,255,255,0.14)`,color:TEXT,padding:'12px 24px',fontSize:'14px',fontWeight:500,borderRadius:9,fontFamily:DM,cursor:'pointer'}}>Ver demo</button>
          </div>
          <p style={{marginTop:12,fontSize:'12px',color:MUTED,fontFamily:DM}}>Sin tarjeta de crédito · <span style={{color:'rgba(93,232,180,.75)'}}>Plan free disponible</span></p>
        </div>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:6,padding:'24px 32px 32px',flexWrap:'wrap'}}>
          {[
            ...
            <div key={p.name} style={{display:'flex',alignItems:'center',gap:7,background:SURF,border:`0.5px solid ${BOR}`,padding:'7px 13px',borderRadius:20,fontSize:'13px',color:MUTED,fontFamily:DM}}>{p.icon}{p.name}</div>
          ))}
          <div style={{display:'flex',alignItems:'center',gap:7,background:'rgba(124,110,245,0.05)',border:`0.5px solid rgba(124,110,245,0.2)`,padding:'7px 13px',borderRadius:20,fontSize:'13px',color:ACC,fontFamily:DM}}>
        ...
        <div style={{margin:'0 32px 60px',background:SURF,border:`0.5px solid ${BOR}`,borderRadius:16,overflow:'hidden'}}>
          <div style={{padding:'11px 16px',borderBottom:`0.5px solid ${BOR}`,display:'flex',alignItems:'center',gap:8}}>
            ...
            <div style={{flex:1,background:'rgba(255,255,255,0.04)',borderRadius:5,padding:'4px 10px',fontSize:'11px',color:MUTED,maxWidth:220,margin:'0 auto',textAlign:'center',fontFamily:DM}}>kaan.app/dashboard</div>
          </div>
          <div style={{padding:18}}>
            <div style={{display:'flex',gap:3,marginBottom:14,overflowX:'auto'}}>
              {['Resumen general','Meta Ads','Google Ads','TikTok','Creativos','Audiencias'].map((t,i)=>(
                <div key={t} style={{padding:'6px 13px',borderRadius:6,fontSize:'12px',cursor:'pointer',whiteSpace:'nowrap',background:i===0?ACC:'none',color:i===0?'white':MUTED,fontFamily:DM}}>{t}</div>
              ))}
            </div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:9,marginBottom:13}}>
              {[
                ...
                <div key={m.label} style={{background:SURF2,borderRadius:9,padding:13}}>
                  <div style={{fontSize:'9px',textTransform:'uppercase',letterSpacing:'.08em',color:MUTED,marginBottom:7,fontFamily:DM}}>{m.label}</div>
                  <div style={{fontSize:'20px',fontFamily:SYNE,fontWeight:700,color:m.color,marginBottom:3}}>{m.val}</div>
                  <span style={{fontSize:'9px',padding:'2px 6px',borderRadius:3,background:m.pos?'rgba(93,232,180,0.09)':'rgba(255,255,255,0.05)',color:m.pos?ACC2:MUTED,fontFamily:DM}}>{m.badge}</span>
                </div>
              ))}
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:9}}>
              <div style={{background:SURF2,borderRadius:9,padding:13}}>
                <div style={{fontSize:'9px',color:MUTED,marginBottom:9,textTransform:'uppercase',letterSpacing:'.06em',fontFamily:DM}}>Inversión por plataforma — 30 días</div>
              ...
              <div style={{background:SURF2,borderRadius:9,padding:13}}>
                <div style={{fontSize:'9px',color:MUTED,marginBottom:9,textTransform:'uppercase',letterSpacing:'.06em',fontFamily:DM}}>Top campañas activas</div>
                <div style={{display:'flex',flexDirection:'column',gap:5}}>
                  {CAMPAIGNS.map(c=>(
                    <div key={c.name} style={{display:'flex',alignItems:'center',gap:7,padding:'5px 7px',background:'rgba(255,255,255,0.025)',borderRadius:5}}>
                      ...
                      <span style={{fontSize:'11px',color:MUTED,fontFamily:DM}}>{c.val}</span>
                      <span style={{fontSize:'9px',padding:'2px 5px',borderRadius:3,background:c.tagBg,color:c.tagColor,fontFamily:DM}}>{c.tag}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div style={{padding:'56px 32px',maxWidth:1080,margin:'0 auto'}}>
          <div style={{fontSize:'11px',textTransform:'uppercase',letterSpacing:'.1em',color:ACC,marginBottom:10,fontFamily:DM}}>Funciones</div>
          <h2 className="sh">Todo lo que necesita<br/>un trafficker profesional</h2>
          <p style={{fontSize:'15px',color:MUTED,maxWidth:520,lineHeight:1.75,marginBottom:36,fontFamily:DM}}>Desde análisis de creativos hasta reportes automáticos — diseñado por media buyers, para media buyers.</p>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',gap:14}}>
            {FEATURES.map(f=>(
              <div key={f.title} style={{background:SURF,border:`0.5px solid ${BOR}`,borderRadius:12,padding:20}}>
                <div style={{width:34,height:34,borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',marginBottom:12,background:f.color}}>{f.icon}</div>
                <div style={{fontFamily:SYNE,fontSize:'14px',fontWeight:600,marginBottom:7,color:TEXT}}>{f.title}</div>
                <div style={{fontSize:'13px',color:MUTED,lineHeight:1.6,fontFamily:DM}}>{f.desc}</div>
                <span style={{display:'inline-block',marginTop:9,fontSize:'10px',padding:'3px 7px',borderRadius:4,background:'rgba(124,110,245,0.09)',color:ACC,fontFamily:DM}}>{f.tag}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{padding:'44px 32px',background:SURF,borderTop:`0.5px solid ${BOR}`,borderBottom:`0.5px solid ${BOR}`}}>
          <div style={{maxWidth:860,margin:'0 auto'}}>
            <div style={{fontSize:'11px',textTransform:'uppercase',letterSpacing:'.1em',color:ACC,marginBottom:10,textAlign:'center',fontFamily:DM}}>Integraciones</div>
            <div style={{fontFamily:SYNE,fontSize:'24px',fontWeight:700,letterSpacing:'-.02em',color:TEXT,textAlign:'center'}}>Conecta donde ya estás invirtiendo</div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(5,minmax(0,1fr))',gap:10,marginTop:26}}>
              {[
                ...
                <div key={p.name} style={{background:SURF2,border:`0.5px ${p.dashed?'dashed':'solid'} ${BOR}`,borderRadius:10,padding:'14px 10px',textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center',gap:6}}>
                  {p.icon}
                  <div style={{fontSize:'12px',color:MUTED,fontFamily:DM}}>{p.name}</div>
                  <span style={{fontSize:'10px',padding:'2px 7px',borderRadius:10,background:p.status==='Disponible'?'rgba(93,232,180,0.08)':'rgba(255,255,255,0.05)',color:p.status==='Disponible'?ACC2:MUTED,fontFamily:DM}}>{p.status}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',border:`0.5px solid ${BOR}`,borderRadius:12,overflow:'hidden'}}>
            {[{n:'3.2x',l:'más rápido que reportes manuales'},{n:'40%',l:'menos tiempo en análisis semanales'},{n:'∞',l:'cuentas de clientes por workspace'}].map((s,i)=>(
              <div key={s.n} style={{padding:'24px 20px',borderRight:i<2?`0.5px solid ${BOR}`:'none',textAlign:'center'}}>
                <div style={{fontFamily:SYNE,fontSize:'28px',fontWeight:800,color:ACC}}>{s.n}</div>
                <div style={{fontSize:'13px',color:MUTED,marginTop:4,fontFamily:DM}}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{padding:'0 32px 56px',maxWidth:1080,margin:'0 auto'}}>
          <div style={{fontSize:'11px',textTransform:'uppercase',letterSpacing:'.1em',color:ACC,marginBottom:10,fontFamily:DM}}>Precios</div>
          <h2 className="sh">Simple y sin sorpresas</h2>
          <p style={{fontSize:'15px',color:MUTED,maxWidth:520,lineHeight:1.75,marginBottom:34,fontFamily:DM}}>Empieza gratis y escala cuando lo necesites.</p>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,maxWidth:660}}>
            <div style={{background:SURF,border:`0.5px solid ${BOR}`,borderRadius:14,padding:24}}>
              <div style={{fontSize:'13px',color:MUTED,marginBottom:9,fontFamily:DM}}>Free</div>
              <div style={{display:'flex',alignItems:'baseline',gap:4}}>
                <span style={{fontFamily:SYNE,fontSize:'32px',fontWeight:800,letterSpacing:'-0.02em',color:TEXT}}><sup style={{fontSize:'14px',fontWeight:400}}>$</sup>0</span>
                <span style={{fontSize:'13px',color:MUTED,fontFamily:DM}}>/ mes</span>
              </div>
              ...
              <ul style={{listStyle:'none',display:'flex',flexDirection:'column',gap:9}}>
                {[{t:'1 cuenta conectada',y:true},{t:'Dashboard multi-métrica',y:true},{t:'Historial 30 días',y:true},{t:'Reportes PDF',y:false},{t:'Alertas',y:false},{t:'Multi-cuenta',y:false}].map(f=>(
                  <li key={f.t} style={{display:'flex',alignItems:'center',gap:8,fontSize:'13px',color:f.y?TEXT:MUTED,fontFamily:DM}}>{f.y?CHECK:CROSS}{f.t}</li>
                ))}
              </ul>
              <button className="bh" onClick={()=>router.push('/registro')} style={{width:'100%',marginTop:18,padding:'10px',borderRadius:8,fontSize:'14px',fontWeight:500,background:'none',border:`0.5px solid ${BOR}`,color:TEXT,fontFamily:DM,cursor:'pointer'}}>Empezar gratis</button>
            </div>
            <div style={{background:SURF,border:`0.5px solid rgba(124,110,245,0.38)`,borderRadius:14,padding:24,overflow:'hidden',position:'relative'}}>
              <div style={{position:'absolute',top:16,right:-28,background:ACC,color:'white',fontSize:'9px',padding:'3px 34px',transform:'rotate(35deg)',letterSpacing:'.04em',fontFamily:DM}}>Más popular</div>
              <div style={{fontSize:'13px',color:MUTED,marginBottom:9,fontFamily:DM}}>Pro</div>
              <div style={{display:'flex',alignItems:'baseline',gap:4}}>
                <span style={{fontFamily:SYNE,fontSize:'32px',fontWeight:800,letterSpacing:'-0.02em',color:TEXT}}><sup style={{fontSize:'14px',fontWeight:400}}>$</sup>29</span>
                <span style={{fontSize:'13px',color:MUTED,fontFamily:DM}}>/ mes</span>
              </div>
              <hr style={{border:'none',borderTop:`0.5px solid ${BOR}`,margin:'16px 0'}}/>
              <ul style={{listStyle:'none',display:'flex',flexDirection:'column',gap:9}}>
                {['Cuentas ilimitadas','Todas las plataformas','Historial 12 meses','Reportes PDF white-label','Alertas inteligentes','Soporte prioritario'].map(f=>(
                  <li key={f} style={{display:'flex',alignItems:'center',gap:8,fontSize:'13px',color:TEXT,fontFamily:DM}}>{CHECK}{f}</li>
                ))}
              </ul>
              <button className="bh" onClick={()=>router.push('/registro')} style={{width:'100%',marginTop:18,padding:'10px',borderRadius:8,fontSize:'14px',fontWeight:500,background:ACC,color:'white',border:'none',fontFamily:DM,cursor:'pointer'}}>Empezar con Pro</button>
            </div>
          </div>
        </div>
        ...
        <div style={{padding:'56px 32px',maxWidth:700,margin:'0 auto'}}>
          <div style={{fontSize:'11px',textTransform:'uppercase',letterSpacing:'.1em',color:ACC,marginBottom:10,textAlign:'center',fontFamily:DM}}>FAQ</div>
          <h2 className="sh" style={{textAlign:'center',marginBottom:34}}>Preguntas frecuentes</h2>
          <div>
            {FAQS.map((faq,i)=>(
              <div key={i} style={{borderBottom:`0.5px solid ${BOR}`}}>
                <div onClick={()=>setOpenFaq(openFaq===i?null:i)} style={{padding:'17px 0',fontSize:'15px',fontWeight:500,color:TEXT,cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,fontFamily:DM}}>
                  {faq.q}
                ...
                {openFaq===i && <div style={{fontSize:'14px',color:MUTED,lineHeight:1.7,paddingBottom:16,fontFamily:DM}}>{faq.a}</div>}
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{padding:'26px 32px',borderTop:`0.5px solid ${BOR}`,display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
          <div style={{fontSize:'13px',color:MUTED,fontFamily:DM}}>© 2026 Kaan · Todos los derechos reservados</div>
          <div style={{display:'flex',gap:18,fontSize:'13px'}}>
            {['Privacidad','Términos','Contacto'].map(l=><a key={l} className="nh" style={{color:MUTED,fontFamily:DM}}>{l}</a>)}
          </div>
        </div>
