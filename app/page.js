...
const FEATURES = [
  ...
  { color: 'pink', title: 'Alertas inteligentes', desc: 'Te avisamos cuando el CPA sube, el presupuesto se agota o el ROAS cae. Actúa antes de que tu cliente te llame.', tag: 'Automatización', icon: <svg viewBox="0 0 24 24" fill="none" stroke="#ed93b1" strokeWidth="1.8" width="17" height="17"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg> },
  { color: 'green', title: 'Multi-cuenta', desc: 'Gestiona todos tus clientes desde un panel. Cambia entre cuentas al instante sin desconectarte ni volver a hacer login.', tag: 'Agencias', icon: <svg viewBox="0 0 24 24" fill="none" stroke="#97c459" strokeWidth="1.8" width="17" height="17"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2"/><line x1="12" y1="12" x2="12" y2="16"/><line x1="10" y1="14" x2="14" y2="14"/></svg> },
]
...
const FAQS = [
  ...
  { q: '¿Puedo cancelar en cualquier momento?', a: 'Sí, sin compromisos ni penalizaciones. Si cancelas, conservas acceso Pro hasta el final del periodo pagado y luego pasas al plan free automáticamente. Tus datos se quedan guardados.' },
  { q: '¿Qué pasa con mis datos si cancelo?', a: 'Tus datos históricos se conservan 90 días después de cancelar. Puedes exportarlos en cualquier momento antes de que eso pase. No vendemos ni compartimos tus datos con terceros.' },
]
...
const FEAT_BG = { purple: 'rgba(124,110,245,0.12)', teal: 'rgba(93,232,180,0.09)', amber: 'rgba(239,159,39,0.09)', blue: 'rgba(55,138,221,0.1)', pink: 'rgba(212,83,126,0.09)', green: 'rgba(99,153,34,0.09)' }
  ...
  return (
    <div style={{fontFamily:DM,fontSize:'15px',lineHeight:'1.6',color:TEXT,background:BG,minHeight:'100vh'}}>
      <style>{`
        ...
        .kl h1 { font-family: ${SYNE}; font-size: 48px; font-weight: 800; line-height: 1.1; letter-spacing: -0.03em; margin-bottom: 18px; color: ${TEXT}; }
        .kl h2 { font-family: ${SYNE}; font-size: 30px; font-weight: 700; line-height: 1.2; letter-spacing: -0.02em; margin-bottom: 12px; color: ${TEXT}; }
        .kl a { text-decoration: none; color: ${MUTED}; cursor: pointer; }
        .kl a:hover { color: ${TEXT}; }
        .kl button { font-family: ${DM}; cursor: pointer; }
        .kl .h1-acc1 { color: ${ACC}; }
        .kl .h1-acc2 { color: ${ACC2}; }
        .nav-link-hover:hover { color: ${TEXT} !important; }
        .pp-hover:hover { border-color: rgba(255,255,255,0.14) !important; color: ${TEXT} !important; }
        .feat-hover:hover { border-color: rgba(255,255,255,0.12) !important; }
        .faq-q-el:hover { opacity: 0.8; }
      `}</style>
      ...
      <div className="kl">
        ...
        <nav style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'18px 32px',borderBottom:`0.5px solid ${BOR}`,background:'rgba(10,10,12,0.94)',backdropFilter:'blur(10px)',position:'sticky',top:0,zIndex:100}}>
          <div style={{display:'flex',alignItems:'center',gap:9,fontFamily:SYNE,fontWeight:700,fontSize:'17px',color:TEXT}}>
            <div style={{width:30,height:30,background:ACC,borderRadius:7,display:'flex',alignItems:'center',justifyContent:'center'}}>
          ...
          <div style={{display:'flex',gap:26,fontSize:'14px'}}>
            {NAV_LINKS.map(l => <a key={l} className="nav-link-hover" style={{color:MUTED}}>{l}</a>)}
          </div>
          <div style={{display:'flex',gap:10,alignItems:'center'}}>
            <button style={{background:'none',border:`0.5px solid ${BOR}`,color:MUTED,padding:'8px 18px',borderRadius:8,fontSize:'13px'}}>Iniciar sesión</button>
            <button style={{background:ACC,color:'white',border:'none',padding:'9px 20px',borderRadius:8,fontSize:'13px',fontWeight:500}}>Empezar gratis</button>
          </div>
        </nav>

        {/* HERO */}
        <div style={{padding:'80px 32px 48px',textAlign:'center',maxWidth:780,margin:'0 auto'}}>
          <div style={{display:'inline-flex',alignItems:'center',gap:7,background:'rgba(124,110,245,0.09)',border:`0.5px solid rgba(124,110,245,0.25)`,color:ACC,fontSize:'12px',padding:'5px 13px',borderRadius:20,marginBottom:26,letterSpacing:'.02em'}}>
            <span style={{width:5,height:5,borderRadius:'50%',background:ACC2,display:'inline-block'}}/>
            Analytics para traffickers
          </div>
          <h1>Tus campañas, <span className="h1-acc1">todas</span> en <span className="h1-acc2">un solo lugar</span></h1>
          <p style={{fontSize:'16px',color:MUTED,maxWidth:520,margin:'0 auto 32px',lineHeight:1.75}}>
          ...
          <div style={{display:'flex',gap:11,justifyContent:'center'}}>
            <button style={{background:ACC,color:'white',border:'none',padding:'13px 26px',fontSize:'14px',fontWeight:500,borderRadius:9,display:'flex',alignItems:'center',gap:7}}>
              <svg viewBox="0 0 16 16" fill="none" width="13" height="13"><path d="M8 2L13 8L8 14L3 8Z" fill="white"/></svg>
              Empezar gratis
            </button>
            <button style={{background:'none',border:`0.5px solid rgba(255,255,255,0.14)`,color:TEXT,padding:'13px 26px',fontSize:'14px',fontWeight:500,borderRadius:9}}>Ver demo</button>
          </div>
          <p style={{marginTop:13,fontSize:'12px',color:MUTED}}>Sin tarjeta de crédito · <span style={{color:'rgba(93,232,180,.75)'}}>Plan free disponible</span></p>
        </div>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:6,padding:'28px 32px',flexWrap:'wrap'}}>
          {PLATFORMS_STRIP.map(p => (
            <div key={p.name} className="pp-hover" style={{display:'flex',alignItems:'center',gap:7,background:SURF,border:`0.5px solid ${BOR}`,padding:'7px 13px',borderRadius:20,fontSize:'13px',color:MUTED}}>
              {p.icon}{p.name}
            </div>
          ))}
        ...
        {/* DASHBOARD MOCKUP */}
        <div style={{margin:'0 32px 64px',background:SURF,border:`0.5px solid ${BOR}`,borderRadius:16,overflow:'hidden'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:9,marginBottom:13}}>
              {[
                ...
                <div key={m.label} style={{background:SURF2,borderRadius:9,padding:13}}>
                  ...
                  <div style={{fontSize:'21px',fontFamily:SYNE,fontWeight:700,color:m.color,marginBottom:3}}>{m.val}</div>
        ...
        <div style={{padding:'60px 32px',maxWidth:1080,margin:'0 auto'}}>
          ...
          <h2>Todo lo que necesita<br/>un trafficker profesional</h2>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',gap:14}}>
            {FEATURES.map(f=>(
              <div key={f.title} className="feat-hover" style={{background:SURF,border:`0.5px solid ${BOR}`,borderRadius:12,padding:20}}>
                <div style={{width:34,height:34,borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',marginBottom:12,background:FEAT_BG[f.color]}}>{f.icon}</div>
        ...
        <div style={{padding:'44px 32px',background:SURF,borderTop:`0.5px solid ${BOR}`,borderBottom:`0.5px solid ${BOR}`}}>
          <div style={{maxWidth:860,margin:'0 auto'}}>
            ...
            <div style={{fontFamily:SYNE,fontSize:'26px',fontWeight:700,letterSpacing:'-.02em',color:TEXT,textAlign:'center'}}>Conecta donde ya estás invirtiendo</div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(5,minmax(0,1fr))',gap:10,marginTop:28}}>
              {[
                ...
                <div key={p.name} style={{background:SURF2,border:`0.5px ${p.dashed?'dashed':'solid'} ${BOR}`,borderRadius:10,padding:'16px 10px',textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center',gap:7}}>
                  {p.icon}
        ...
        <div style={{padding:'60px 32px',maxWidth:1080,margin:'0 auto'}}>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',border:`0.5px solid ${BOR}`,borderRadius:12,overflow:'hidden'}}>
            {[{n:'3.2x',l:'más rápido que reportes manuales'},{n:'40%',l:'menos tiempo en análisis semanales'},{n:'∞',l:'cuentas de clientes por workspace'}].map((s,i)=>(
              <div key={s.n} style={{padding:'26px 22px',borderRight:i<2?`0.5px solid ${BOR}`:'none',textAlign:'center'}}>
                <div style={{fontFamily:SYNE,fontSize:'30px',fontWeight:800,color:ACC}}>{s.n}</div>
        ...
        <div style={{padding:'0 32px 60px',maxWidth:1080,margin:'0 auto'}}>
          ...
          <p style={{fontSize:'15px',color:MUTED,maxWidth:520,lineHeight:1.75,marginBottom:36}}>Empieza gratis y escala cuando lo necesites.</p>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,maxWidth:680}}>
            <div style={{background:SURF,border:`0.5px solid ${BOR}`,borderRadius:14,padding:26}}>
              <div style={{fontSize:'13px',color:MUTED,marginBottom:9}}>Free</div>
              <div style={{display:'flex',alignItems:'baseline',gap:4}}>
                <span style={{fontFamily:SYNE,fontSize:'34px',fontWeight:800,letterSpacing:'-0.02em'}}><sup style={{fontSize:'15px',fontWeight:400}}>$</sup>0</span>
                <span style={{fontSize:'13px',color:MUTED}}>/ mes</span>
              </div>
              <hr style={{border:'none',borderTop:`0.5px solid ${BOR}`,margin:'18px 0'}}/>
              ...
              <button style={{width:'100%',marginTop:20,padding:11,borderRadius:8,fontSize:'14px',fontWeight:500,background:'none',border:`0.5px solid ${BOR}`,color:TEXT}}>Empezar gratis</button>
            </div>
            <div style={{background:SURF,border:`0.5px solid rgba(124,110,245,0.38)`,borderRadius:14,padding:26,overflow:'hidden',position:'relative'}}>
              ...
              <div style={{display:'flex',alignItems:'baseline',gap:4}}>
                <span style={{fontFamily:SYNE,fontSize:'34px',fontWeight:800,letterSpacing:'-0.02em'}}><sup style={{fontSize:'15px',fontWeight:400}}>$</sup>29</span>
                <span style={{fontSize:'13px',color:MUTED}}>/ mes</span>
              </div>
              <hr style={{border:'none',borderTop:`0.5px solid ${BOR}`,margin:'18px 0'}}/>
              <ul style={{listStyle:'none',display:'flex',flexDirection:'column',gap:9}}>
              ...
              <button style={{width:'100%',marginTop:20,padding:11,borderRadius:8,fontSize:'14px',fontWeight:500,background:ACC,color:'white',border:'none'}}>Empezar con Pro</button>
            </div>
          </div>
        </div>
        ...
        <div style={{padding:'60px 32px',maxWidth:720,margin:'0 auto'}}>
          ...
          <h2 style={{textAlign:'center',marginBottom:36}}>Preguntas frecuentes</h2>
          <div>
            {FAQS.map((faq,i)=>(
              <div key={i} style={{borderBottom:`0.5px solid ${BOR}`}}>
                <div className="faq-q-el" onClick={()=>setOpenFaq(openFaq===i?null:i)} style={{padding:'18px 0',fontSize:'15px',fontWeight:500,color:TEXT,cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',gap:12}}>
                  {faq.q}
        ...
        {/* FOOTER */}
        <div style={{padding:'28px 32px',borderTop:`0.5px solid ${BOR}`,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          ...
          <div style={{display:'flex',gap:18,fontSize:'13px'}}>
            {['Privacidad','Términos','Contacto'].map(l=><a key={l} style={{color:MUTED}}>{l}</a>)}
          </div>
        </div>
