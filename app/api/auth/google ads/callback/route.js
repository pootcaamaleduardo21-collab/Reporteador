...
export async function GET(request) {
  ...
  try {
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'https://reporteador.vercel.app'}/dashboard?success=google_ads_connected`
    );
  } catch (error) {
    console.error('OAuth error:', error);
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'https://reporteador.vercel.app'}/dashboard?error=oauth_error`
    );
  }
}